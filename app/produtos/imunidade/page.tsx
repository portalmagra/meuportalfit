...
export default function ImunidadePage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (error) {
          ...
          if (storedProducts) {
            ...
            const imunidadeProducts = allProducts.filter((product: any) => 
              product.categoryId === 'imunidade'
            )
            console.log('🔄 Fallback para localStorage:', imunidadeProducts.length, 'produtos')
            setProducts(imunidadeProducts)
          }
        } else {
    ...
    try {
      ...
      console.log('❌ BroadcastChannel não suportado na página imunidade:', error)
    }
  }, [])
                ...
                {products.map((product) => (
                  <div key={product.id} style={{
                    ...
                    <div style={{
                      ...
                      <a 
                        href={`/produtos/imunidade/${product.slug || product.id}`} 
                        style={{ 
                          textDecoration: 'none', 
                          flex: 1,
                          display: 'block',

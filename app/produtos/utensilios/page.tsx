...
export default function UtensíliosdeSuportePage() {
  const [language, setLanguage] = useState<'pt' | 'es' | 'en'>('pt')
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Carregar produtos da categoria "utensilios" do Supabase
    const loadProducts = async () => {
      try {
        console.log('🔄 Carregando produtos do Supabase...')
        
        // Buscar produtos da categoria utensilios no Supabase
        const { data: products, error } = await supabase
          .from('products')
          .select('*')
          .eq('category_id', 'utensilios')
        ...
        if (error) {
          ...
          if (storedProducts) {
            ...
            const utensiliosProducts = allProducts.filter((product: any) => 
              product.categoryId === 'utensilios'
            )
            console.log('🔄 Fallback para localStorage:', utensiliosProducts.length, 'produtos')
            setProducts(utensiliosProducts)
          }
        } else {
    ...
    try {
      ...
      console.log('📡 Escutando sincronização na página utensilios')
      ...
      console.log('❌ BroadcastChannel não suportado na página utensilios:', error)
    }
  }, [])
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '20px', fontWeight: 'bold' }}>
              🏋️ Suporte para Utensílios de Suporte
            </h1>
            <p style={{ fontSize: '1.2rem', marginBottom: '30px', opacity: 0.9 }}>
              Ferramentas e equipamentos para fitness e saúde
            </p>
                ...
                {products.map((product) => (
                  <div key={product.id} style={{
                    ...
                    <div style={{
                      ...
                      <a 
                        href={`/produtos/utensilios/${product.slug || product.id}`} 
                        style={{ 
                          textDecoration: 'none', 
                          flex: 1,
                          display: 'block',

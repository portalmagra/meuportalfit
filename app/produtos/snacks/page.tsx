...
export default function SnackSaudávelPage() {
  const [language, setLanguage] = useState<'pt' | 'es' | 'en'>('pt')
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Carregar produtos da categoria "snacks" do Supabase
    const loadProducts = async () => {
      try {
        console.log('🔄 Carregando produtos do Supabase...')
        
        // Buscar produtos da categoria snacks no Supabase
        const { data: products, error } = await supabase
          .from('products')
          .select('*')
          .eq('category_id', 'snacks')
        ...
        if (error) {
          ...
          if (storedProducts) {
            ...
            const snacksProducts = allProducts.filter((product: any) => 
              product.categoryId === 'snacks'
            )
            console.log('🔄 Fallback para localStorage:', snacksProducts.length, 'produtos')
            setProducts(snacksProducts)
          }
        } else {
    ...
    try {
      ...
      console.log('📡 Escutando sincronização na página snacks')
      ...
      console.log('❌ BroadcastChannel não suportado na página snacks:', error)
    }
  }, [])
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '20px', fontWeight: 'bold' }}>
              🍎 Suporte para Snack Saudável
            </h1>
            <p style={{ fontSize: '1.2rem', marginBottom: '30px', opacity: 0.9 }}>
              Lanches nutritivos e funcionais
            </p>
                ...
                {products.map((product) => (
                  <div key={product.id} style={{
                    ...
                    <div style={{
                      ...
                      <a 
                        href={`/produtos/snacks/${product.slug || product.id}`} 
                        style={{ 
                          textDecoration: 'none', 
                          flex: 1,
                          display: 'block',

...
export default function MercadodeHomensPage() {
  const [language, setLanguage] = useState<'pt' | 'es' | 'en'>('pt')
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Carregar produtos da categoria "homens" do Supabase
    const loadProducts = async () => {
      try {
        console.log('🔄 Carregando produtos do Supabase...')
        
        // Buscar produtos da categoria homens no Supabase
        const { data: products, error } = await supabase
          .from('products')
          .select('*')
          .eq('category_id', 'homens')
        ...
        if (error) {
          ...
          if (storedProducts) {
            ...
            const homensProducts = allProducts.filter((product: any) => 
              product.categoryId === 'homens'
            )
            console.log('🔄 Fallback para localStorage:', homensProducts.length, 'produtos')
            setProducts(homensProducts)
          }
        } else {
    ...
    try {
      ...
      console.log('📡 Escutando sincronização na página homens')
      ...
      console.log('❌ BroadcastChannel não suportado na página homens:', error)
    }
  }, [])
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '20px', fontWeight: 'bold' }}>
              👨 Suporte para Mercado de Homens
            </h1>
            <p style={{ fontSize: '1.2rem', marginBottom: '30px', opacity: 0.9 }}>
              Produtos específicos para saúde masculina
            </p>
                ...
                {products.map((product) => (
                  <div key={product.id} style={{
                    ...
                    <div style={{
                      ...
                      <a 
                        href={`/produtos/homens/${product.slug || product.id}`} 
                        style={{ 
                          textDecoration: 'none', 
                          flex: 1,
                          display: 'block',

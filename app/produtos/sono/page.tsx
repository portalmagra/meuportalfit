...
export default function QualidadedoSonoPage() {
  const [language, setLanguage] = useState<'pt' | 'es' | 'en'>('pt')
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Carregar produtos da categoria "sono" do Supabase
    const loadProducts = async () => {
      try {
        console.log('🔄 Carregando produtos do Supabase...')
        
        // Buscar produtos da categoria sono no Supabase
        ...
        if (error) {
          ...
          if (storedProducts) {
            ...
            const sonoProducts = allProducts.filter((product: any) => 
              product.categoryId === 'sono'
            )
            console.log('🔄 Fallback para localStorage:', sonoProducts.length, 'produtos')
            setProducts(sonoProducts)
          }
        } else {
    ...
    try {
      ...
      console.log('📡 Escutando sincronização na página sono')
      ...
      console.log('❌ BroadcastChannel não suportado na página sono:', error)
    }
  }, [])
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '20px', fontWeight: 'bold' }}>
              😴 Suporte para Qualidade do Sono
            </h1>
                ...
                {products.map((product) => (
                  <div key={product.id} style={{
                    ...
                    <div style={{
                      ...
                      <a 
                        href={`/produtos/sono/${product.slug || product.id}`} 
                        style={{ 
                          textDecoration: 'none', 
                          flex: 1,
                          display: 'block',
